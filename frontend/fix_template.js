...
const targetStr = `{/* Hidden Print Template */}
      {isPrinting && currentPrintReferral && (
        <div className="hidden print:block absolute inset-0 bg-white z-[9999] p-10 font-sans text-black">
          {/* Style for printing */}
          ...
          <div className="max-w-4xl mx-auto border-b-2 border-black pb-6 mb-8 flex justify-between items-center">
            <div>
               <h1 className="text-2xl font-black uppercase tracking-widest">{queue?.clinicId ? clinicsList.find(c => c.id === queue.clinicId)?.name || 'KLINIK' : 'KLINIK PUSAT'}</h1>
               <p className="text-sm mt-1">Layanan Medis & Konsultasi Spesialis</p>
            </div>
            <div className="text-right">
               <h2 className="text-3xl font-black uppercase tracking-tight">SURAT RUJUKAN</h2>
               <p className="text-sm font-bold uppercase tracking-widest mt-1">NO: REF-{currentPrintReferral.id.split('-')[0].toUpperCase()}</p>
            </div>
          </div>
          
          <div className="max-w-4xl mx-auto mb-8">
            <p className="text-sm mb-4">Kepada Yth.,<br/><strong>Teman Sejawat Dokter / {currentPrintReferral.type === 'INTERNAL' ? \`Poli \${currentPrintReferral.toDepartment?.name}\` : 'Poliklinik Terkait'}</strong><br/>Di Tempat</p>
            <p className="text-sm">Mohon pemeriksaan dan penanganan lebih lanjut terhadap pasien berikut:</p>
          </div>
          
          <div className="max-w-4xl mx-auto mb-8">
            <table className="w-full text-sm">
              <tbody>
                <tr><td className="w-40 py-2 font-bold">Nama Pasien</td><td className="w-4 py-2">:</td><td className="py-2">{queue.patient.name}</td></tr>
                <tr><td className="w-40 py-2 font-bold">No. Rekam Medis</td><td className="w-4 py-2">:</td><td className="py-2">{queue.patient.medicalRecordNo}</td></tr>
                <tr><td className="w-40 py-2 font-bold">Jenis Kelamin</td><td className="w-4 py-2">:</td><td className="py-2">{['L', 'M', 'Laki-laki'].includes(queue.patient.gender) ? 'Laki-laki' : 'Perempuan'}</td></tr>
                <tr><td className="w-40 py-2 font-bold">Tujuan Rujukan</td><td className="w-4 py-2">:</td><td className="py-2 font-black">{currentPrintReferral.type === 'INTERNAL' ? \`\${currentPrintReferral.toClinic?.name} - Poli \${currentPrintReferral.toDepartment?.name}\` : currentPrintReferral.toHospitalName}</td></tr>
              </tbody>
            </table>
          </div>
          
          <div className="max-w-4xl mx-auto mb-8 border border-black p-6">
             <h3 className="text-sm font-black uppercase mb-4 border-b border-black pb-2">Informasi Klinis</h3>
             <div className="grid grid-cols-2 gap-8 text-sm">
                <div>
                   <p className="font-bold mb-1">Anamnesa (Subjective):</p>
                   <p className="whitespace-pre-wrap italic mb-4">{subjective || '-'}</p>
                   <p className="font-bold mb-1">Pemeriksaan Fisik (Objective):</p>
                   <p className="whitespace-pre-wrap italic">{objective || '-'}</p>
                </div>
                <div>
                   <p className="font-bold mb-1">Diagnosa Sementara (Assessment):</p>
                   <p className="whitespace-pre-wrap font-bold mb-4">{diagnosis || '-'}</p>
                   <p className="font-bold mb-1">Terapi Diberikan:</p>
                   <p className="whitespace-pre-wrap italic">{treatmentPlan || '-'}</p>
                </div>
             </div>
          </div>
          
          <div className="max-w-4xl mx-auto mb-12">
             <h3 className="text-sm font-black uppercase mb-2">Catatan Rujukan:</h3>
             <p className="whitespace-pre-wrap text-sm italic">{currentPrintReferral.notes || '-'}</p>
          </div>
          
          <div className="max-w-4xl mx-auto flex justify-end">
             <div className="text-center w-64">
                <p className="text-sm mb-20">{new Date().toLocaleDateString('id-ID', { day: 'numeric', month: 'long', year: 'numeric' })}<br/>Dokter Perujuk,</p>
