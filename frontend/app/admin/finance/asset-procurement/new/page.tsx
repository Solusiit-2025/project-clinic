...
export default function NewAssetProcurementPage() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      await api.post('/finance/asset-procurement', {
        branchId: activeClinicId,
        supplierName,
        paymentType,
        downPayment: paymentType === 'INSTALLMENT' ? downPayment : 0,
        totalInterest: paymentType === 'INSTALLMENT' ? totalInterest : 0,
        tenorMonths: paymentType === 'INSTALLMENT' ? tenorMonths : 0,
        items
      })
  ...
  return (
    <div className="p-6 w-full space-y-6">
      <div className="flex items-center gap-4 bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
        ...
        <div>
          <h1 className="text-2xl font-black text-gray-800 tracking-tight">Buat PO Aset Baru</h1>
          <p className="text-gray-500 mt-1">Buat dokumen pemesanan aset baru dengan pembayaran lunas atau cicilan.</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-3 gap-6">
        ...
        <div className="col-span-1 space-y-6">
          <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 space-y-5">
            ...
            className="flex items-center justify-center gap-2 w-full bg-primary hover:bg-primary/90 text-white p-4 rounded-xl font-bold text-lg transition-all shadow-lg shadow-primary/30 disabled:opacity-70"
          >
            <Save className="w-6 h-6" />
            {isSubmitting ? 'Menyimpan...' : 'Simpan PO Aset'}
          </button>
          </div>
        </div>
      </div>
