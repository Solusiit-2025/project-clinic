...
export default function AdminProfilePage() {
  ...
  const roleConfig = ROLE_COLORS[user?.role ?? 'STAFF'] ?? ROLE_COLORS.STAFF
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs font-bold text-gray-400 uppercase tracking-widest mt-1">
            Informasi akun dan hak akses operasional Anda
          </p>
        </div>
        ...
        {activeTab === 'info' ? (
            ...
            <div className="lg:col-span-2 space-y-6">
              ...
              <div className="bg-white rounded-3xl border border-gray-100 shadow-sm p-6">
                ...
                {user?.role === 'SUPER_ADMIN' ? (
                  <div className="bg-gradient-to-r from-amber-500 to-orange-500 rounded-2xl p-5 border border-amber-400 text-white shadow-lg shadow-amber-100">
                    <div className="flex items-center gap-3 mb-2">
                      <FiShield className="w-5 h-5 animate-pulse" />
                      <p className="font-black text-sm uppercase tracking-wider">Super Administrator Privileged Mode</p>
                    </div>
                    <p className="text-xs font-medium text-white/95 leading-relaxed">
                      Akun Anda memegang status kepemilikan tertinggi. Anda memiliki izin tak terbatas untuk mengakses seluruh modul transaksi, mengedit rekam medis pasien, mengatur konfigurasi akuntansi buku besar, serta mengelola hak akses seluruh staff klinik di database.
                    </p>
                  </div>
                ) : (
                  <div className="space-y-4">
                    <div className="p-3.5 bg-blue-50/50 border border-blue-100 rounded-xl">
                      <p className="text-xs font-medium text-blue-700 leading-relaxed">
                        Berikut adalah modul-modul sistem yang dapat Anda buka dan kelola berdasarkan izin role Anda saat ini:
                      </p>
                    </div>
                    ...
                    {user?.permissions && user.permissions.length > 0 ? (
                      ...
                      <div className="p-6 text-center rounded-2xl border border-dashed border-gray-200">
                        <FiLock className="w-8 h-8 text-gray-300 mx-auto mb-2" />
                        <p className="text-xs font-bold text-gray-400 uppercase tracking-widest">Belum ada modul aktif</p>
                        <p className="text-[10px] text-gray-300 font-semibold mt-1">Gunakan role Super Admin untuk mengaktifkan modul.</p>
                      </div>
                    )}
                  </div>
                )}
          ...
          >
            ...
            <div className="bg-white rounded-3xl border border-gray-100 shadow-sm p-6 relative overflow-hidden h-fit">
              ...
              <h3 className="text-sm font-black text-gray-900 uppercase tracking-tight">Kemanan Akun Anda</h3>
